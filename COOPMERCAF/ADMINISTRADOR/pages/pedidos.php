...
<body>
    ...
    include('abrir_conexion.php');
            $consulta = mysqli_query($conexion, "SELECT * FROM $tabla_db7 ORDER BY id desc");
    include('cerrar_conexion.php')
        ...
        while ($fila = mysqli_fetch_array($consulta)) {
            include("abrir_conexion.php");
                $consultar_categorias = mysqli_query($conexion,"SELECT * from $tabla_db2 WHERE id='$fila[categorias_productos_id]'");
                    $categoria = mysqli_fetch_array($consultar_categorias);
                $consultar_productos = mysqli_query($conexion,"SELECT * from $tabla_db8 WHERE id='$fila[productos_id]'");
                    $productos = mysqli_fetch_array($consultar_productos);
                $consultar_unidad_medida = mysqli_query($conexion,"SELECT * from $tabla_db12 WHERE id='$fila[unidades_medida_id]'");
                    $unidad_medida = mysqli_fetch_array($consultar_unidad_medida);
                $consultar_cliente = mysqli_query($conexion,"SELECT * from $tabla_db3 WHERE id='$fila[clientes_id]'");
                    $cliente = mysqli_fetch_array($consultar_cliente);
            include('cerrar_conexion.php');
            echo "
                    <tr>
                        <td><center>".$categoria['categoria']."</center</td>
                        <td><center>".$productos['nombre']."</center></td>
                        <td><center>".$unidad_medida['unidad_medida']."</center></td>
                        <td><center>".$fila['cantidad']."</center></td>
                        <td><center>".$fila['fecha']."</center></td>
                        <td><center>".$cliente['nombres']."".$cliente['apellidos']."</center</td>
                        <td><center>".$fila['estado_pedido']."</center</td>
                        <td><center><a href='actualizar_pedidos.php?id=$fila[id]' class='btn btn-success'>Editar</a></center></td>
                        <td><center><a href='eliminar_pedidos.php?id=$fila[id]' class='btn btn-danger'>Eliminar</a></center></td>
                    </tr>
              ";
        }
         echo "</table>";
