...
<body>
    ...
    include('abrir_conexion.php');
            $consulta = mysqli_query($conexion, "SELECT * FROM $tabla_db3 ORDER BY id desc");
    include('cerrar_conexion.php')
        ...
        while ($fila = mysqli_fetch_array($consulta)) {
            include("abrir_conexion.php");
                $consulta_departamento = mysqli_query($conexion, "SELECT departamento FROM $tabla_db4 where id = $fila[departamentos_id]");
                $consulta_municipios = mysqli_query($conexion, "SELECT municipios FROM $tabla_db6 where id = $fila[municipios_id]");
                    $departamento = mysqli_fetch_array($consulta_departamento);
                    $municipios = mysqli_fetch_array($consulta_municipios);

            include('cerrar_conexion.php');
            echo "
                    <tr>
                        <td><center>".$fila['nombres']."</center></td>
                        <td><center>".$fila['apellidos']."</center></td>
                        <td><center>".$fila['documento_identidad']."</center></td>
                        <td><center>".$fila['celular']."</center></td>
                        <td><center>".$fila['correo']."</center></td>
                        <td><center>".$fila['direccion']."</center></td>
                        <td><center>".$departamento['departamento']."</center></td>
                        <td><center>".$municipios['municipios']."</center></td>
                        <td><center><a href='actualizar_clientes.php?id=$fila[id]' class='btn btn-success'>Editar</a></center></td>
                        <td><center><a href='eliminar_clientes.php?id=$fila[id]' class='btn btn-danger'>Eliminar</a></center></td>
                    </tr>
              ";
        }
         echo "</table>";
